<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($price) || !is_numeric($price)) {
            $priceError = "Invalid product price!";
        }
        ...
        if (!haveErrors($nameError, $priceError, $imageError)) {
            $preparedStatement = $connection->prepare("SELECT * FROM products WHERE name=?");
            ...
            if ($result->num_rows > 0) {
                $duplicateError = "Product already exists!";
            } else {
                move_uploaded_file($image["tmp_name"], $imageDir);
                $preparedStatement = $connection->prepare("INSERT INTO products (name, price, image) VALUES (?, ?, ?)");
                        ...
                        <div class="card" style="width: 600px">
                            <div class="card-body">
                                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="POST" enctype="multipart/form-data">
                                    ...
                                    <button type="submit" class="btn btn-primary" style="background-color: #d3ad7f; border-color: #d3ad7f">Create Product</button>
                                </form>
                            </div>
                        </div>
                    </div>
