<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!haveErrors($titleError, $descriptionError, $imageError)) {
            $preparedStatement = $connection->prepare("SELECT * FROM blogs WHERE title=?");
            $preparedStatement->bind_param("s", $title);
            $preparedStatement->execute();

            $result = $preparedStatement->get_result();

            if ($result->num_rows > 0) {
                $duplicateError = "Blog already exists!";
            } else {
                move_uploaded_file($image["tmp_name"], $imageDir);
                $preparedStatement = $connection->prepare("INSERT INTO blogs (title, description, image) VALUES (?, ?, ?)");
                $preparedStatement->bind_param("sss", $title, $description, $imageDir);
                $preparedStatement->execute();
    ...
    function haveErrors($titleError, $descriptionError, $imageError) {
                            ...
                            <div class="card-body">
                                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        <?php
                                            if (!empty($titleError)) {
                                                echo "<p style='color: red'>$titleError</p>";
                                            }
                                        ?>
                                    ...
                                    <div class="mb-3">
                                        <?php
                                            if (!empty($descriptionError)) {
                                                echo "<p style='color: red'>$descriptionError</p>";
                                            }
                                        ?>
                                        <label for="description" class="form-label">Description</label>
                                        <textarea rows="13" name="description" type="text" class="form-control" id="description"></textarea>
                                    </div>
                                    ...
                                    <button type="submit" class="btn btn-primary" style="background-color: #d3ad7f; border-color: #d3ad7f">Create Blog</button>
                                </form>
                            </div>
                        </div>
                    </div>
